...
if page == "Churn Prediction":
    ...
    with col2 : 
        with st.expander("Internet and Streaming services"): 
            ...
            online_backup = st.selectbox("online_backup" , ["No", "Yes", "No internet service"])
            device_protection = st.selectbox("device_protection" ,["No", "Yes", "No internet service"] )
            tech_support = st.selectbox("tech_support" ,["No", "Yes", "No internet service"] )
    ...
    with st.expander("Other Information"): 
        ...
        monthly_charges = st.slider("Monthly Charges" , min_value=0.0 , max_value=1000.0 , value=50.0)
        total_charges = st.slider("Total Charges" , min_value=0.0, max_value=20000.0, value=0.0)
    ...
    input_df = pd.DataFrame([{
        ...
        "TechSupport" : tech_support , "StreamingTV" : streaming_tv , "StreamingMovies" : streaming_movies , "Contract": contract , "PaperlessBilling" : paperless_billing , 
...
if uploaded_file : 
    ...
    st.download_button("Download_Predictions" , df_batch.to_csv(index=False) , 'churn_prediction.csv' , "text/csv")
